...
# Sync and start client-side Nsight profiling in a detached tmux session.
# Fetch artifacts with scripts/fetch_vllm_serve_nsys_remote.sh.
# Usage: run_vllm_client_nsys_remote.sh [HOST] [REMOTE_DIR] [extra vllm_main.py client-nsys args...]
...
OUT=${OUT:-results/vllm_bw_client_nsys_${TIMESTAMP}}
SESSION=${TMUX_SESSION:-vllm_bw_client_${TIMESTAMP}}
REMOTE_LOG=${OUT}.remote.log
REMOTE_DIR_ABS=$(ssh "$HOST" "cd $REMOTE_DIR && pwd")

REMOTE_CMD=$(printf \
  'set -o pipefail; cd %q || exit 1; mkdir -p %q; ./scripts/run_vllm_client_nsys.sh %q %s 2>&1 | tee -a %q; code=${PIPESTATUS[0]}; echo "__VLLM_BW_DONE_EXIT_${code}__" | tee -a %q; exit $code' \
