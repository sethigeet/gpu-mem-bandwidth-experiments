...
OUT=${OUT:-results/llm_bw_nsys_${TIMESTAMP}}

ssh "$HOST" "cd $REMOTE_DIR && ./scripts/run_llm_nsys.sh '$OUT' $EXTRA_ARGS"

ssh "$HOST" "cd $REMOTE_DIR && uv run python llm_main.py visualize ${OUT}.sqlite -o ${OUT}.png $EXTRA_ARGS"
mkdir -p "$(dirname "$OUT")"
