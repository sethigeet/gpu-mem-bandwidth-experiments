...
def parse_shape(text: str) -> tuple[int, int, int, int]:
    ...
    if len(parts) != 4:
        raise argparse.ArgumentTypeError("shape must be B,H,S,D, e.g. 4,16,4096,64")
    batch, heads, seq, dim = (int(p) for p in parts)
...
def make_tensors(case: Case, device: torch.device) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    shape = (case.batch, case.heads, case.seq, case.dim)
    return (
